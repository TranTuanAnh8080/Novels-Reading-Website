...
export default function RecommendedAll() {
  ...
  return (
    <div className="min-h-screen bg-[#F3F4F6] dark:bg-gray-900 flex flex-col font-sans transition-colors duration-300">
      <HeaderLoggedIn />
      <div className="flex-grow w-full max-w-[1320px] mx-auto px-4 py-8">
        
        {/* HEADER BAR */}
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 md:p-6 mb-8 flex flex-col md:flex-row justify-between items-center gap-4 transition-colors duration-300">
            <div className="flex items-center gap-3">
                <div className="bg-blue-100 dark:bg-blue-900/30 p-3 rounded-xl text-blue-600 dark:text-blue-400">
                    <LayoutGrid className="w-6 h-6" />
                </div>
                <div>
                    <h1 className="text-xl font-bold text-gray-900 dark:text-white">Kho Truyện Đồ Sộ</h1>
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                        {filteredNovels.length} kết quả tìm thấy
                    </p>
                </div>
            </div>
            ...
            <div className="flex w-full md:w-auto gap-3">
                <div className="relative flex-grow md:w-80">
                    ...
                    <input 
                        ...
                        className="w-full pl-9 pr-4 py-2.5 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white dark:focus:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 transition-all"
                    />
                </div>
                <button className="flex items-center justify-center gap-2 px-4 py-2.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-xl text-sm font-semibold text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors shadow-sm">
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
              {currentNovels.map((novel, index) => (
                <NovelCard key={novel.novelId} novel={novel} index={index + indexOfFirstItem} />
              ))}
            </div>
          ...
          <div className="flex flex-col items-center justify-center py-20">
             <div className="w-24 h-24 bg-gray-200 dark:bg-gray-700 rounded-full flex items-center justify-center mb-4 text-gray-400">
                 <Search className="w-10 h-10" />
             </div>
             <h3 className="text-lg font-bold text-gray-700 dark:text-gray-200">Không tìm thấy truyện</h3>
             <p className="text-gray-500 dark:text-gray-400">Thử tìm kiếm với từ khóa khác xem sao.</p>
          </div>
        )}
      </div>
